...
from core.searchEmail import search_email as sp


def search_email(window, settings):
    search = tk.Toplevel(window)
    search.title("Search Email")
    search.geometry("300x150")

    frame1 = tk.Frame(search, background="White")
    frame1.pack()

    tk.Label(frame1, text="Email adress: ").pack(side=tk.LEFT)
